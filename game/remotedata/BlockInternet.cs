...
namespace ctre_wp7.game.remotedata
{
	...
	internal class BlockInternet : BlockInterface
	{
		...
		{
			string type = this.jblock.getType();
			if (type == "adblock")
			{
				return 0;
			}
		...
		{
			string name = this.jblock.getName();
		...
		{
			string text = this.jblock.getText();
		...
		{
			string number = this.jblock.getNumber();
